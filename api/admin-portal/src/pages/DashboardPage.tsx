...
export function DashboardPage() {
  ...
  return (
    <div>
      ...
      >
        Dashboard
        {currentExamType ? ` \u2014 ${currentExamType.displayName}` : ''}
      </h1>
      ...
      >
        {Object.entries(stats.byDomain).map(([domain, count]) => {
          ...
          const pct = info
            ? Math.min(100, Math.round((count / info.questionCount) * 100))
            : 0;
          return (
            <div
              key={domain}
        ...
        {Object.keys(stats.byDomain).length === 0 && (
          <div style={{ color: colors.subtle, fontSize: 14, padding: 16 }}>
            No questions yet
          </div>
        )}
      </div>
